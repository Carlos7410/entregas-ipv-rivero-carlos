...
export (Vector2) var wander_radius: Vector2 = Vector2(30.0, 30.0)

export (float) var speed: float = 10.0
export (float) var max_speed: float = 60.0
...
func exit() -> void:
	# Al salir del estado walk se limpia los caminos generados
	path = []
...
func update(delta : float) -> void:
	...
	while character.global_position.distance_to(next_point) < pathfinding_step_threshold:
		...
		path.pop_front()
		# Si el camino esta vacío en este punto, se llego al objetivo y vuelve a Idle
		if path.empty():
			emit_signal("finished", "idle")
			return
		# Sino, se indica su siguiente punto al que ir
		next_point = path.front()
		
	character.velocity = (
		character.velocity + 
		character.global_position.direction_to(next_point) * speed).clamped(max_speed)
...
# Manejo de transiciones de animaciones 'intermedias' a animaciones finales
